...
```{block2, type='bdd'}
Les instructions que vous employez dans un learnR peuvent être employées également dans un script d'analyse. Sur base du jeu de données `urchin_bio`, explorez différents graphiques en nuages de points. 
...
```{block2, type='bdd'}
Dans la fenêtre **Console** de RStudio, entrez l'instruction suivante et puis appuyez sur la touche `Entrée` pour ouvrir le tutoriel concernant le nuage de points :
