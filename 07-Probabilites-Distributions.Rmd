...
chart(data = tibble(Quantiles = .x, Prob = .dc(.x)), Prob ~ Quantiles) +
  geom_hline(yintercept = 0, col = "gray") +
  geom_ribbon(x = .x, ymin = 0, ymax = .dc(.x), fill = "gray", alpha = 0.2) +
  geom_line() +
  xlab("Quantile (temps d'observation en min)") +
...
.x2[.x2 < 1 | .x2 > 1.5] <- NA
ch_unif +
  geom_ribbon(x = .x2, ymin = 0, ymax = .d(.x2), fill = "red", alpha = 0.5) +
...
.x3[.x3 > .q3] <- NA
ch_unif +
  geom_ribbon(x = .x3, ymin = 0, ymax = .d(.x3), fill = "red", alpha = 0.5) +
...
chart(data = tibble(Quantiles = .x, Prob = .d(.x)), Prob ~ Quantiles) +
  ...
  geom_ribbon(x = .x1, ymin = 0, ymax = .d(.x1), fill = "orange", alpha = 0.2) +
  geom_line() +
  xlab("Quantile") +
  ylab("Densité de probabilité") +
