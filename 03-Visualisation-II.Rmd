...
```{block2, type='bdd'}
Un projet sur le zooplancton provenant de Madagascar est mis à votre disposition. Utilisez l'URL suivante qui va vous donner accès à votre tâche. **Cette tache est un travail en binome**
...
```{r, fig.cap="Exemple d'histogramme montrant la distribution de la taille d'un échantillon de zooplancton.", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r, fig.cap="Distribution des tailles au sein d'un échantillon de zooplancton"}
...
g1 <- chart(data = df1, ~ x) +
  geom_histogram() +
  labs(x = "Unimodal & symétrique", y = "Occurences")

df2 <- rbind(df1, data_frame(x = rnorm(7000, 60, 5)))
g2 <- chart(data = df2, ~ x) +
  geom_histogram() +
  labs(x = "Bimodal & symétrique", y = "Occurences")

df3 <- data_frame(x = rlnorm(3000, 0, 0.6))
g3 <- chart(data = df3, ~ x) +
  geom_histogram() +
  labs(x = "Unimodal & asymétrique", y = "Occurences")

df4 <- rbind(df2, data_frame(x = rnorm(3000, 80, 5)))
g4 <- chart(data = df4, ~ x) +
  geom_histogram() +
  labs(x = "Multimodal & symétrique", y = "Occurences")
...
```{r histo-classes, fig.cap = "Piège de l'histogramme. A. histogramme initial montrant la répartition des tailles au sein d'organismes planctoniques. B., C., D. Même histogramme que A, mais en modifiant le nombres de classes."}
...
```{r, fig.cap= "Distribution des longueurs de sépales de trois espèces d'iris."}
# Importation du jeu de données
...
```{r , fig.cap= "Distribution de la longueur des sépales de trois espèces d'iris (en employant les facettes pour séparer les espèces)."}
...
```{r, fig.cap= "Distribution des longueurs de sépales de trois espèces d'iris (avec facettes et histogrammes complets grisés en arrière plans)."}
nbins <- 25
...
```{r, fig.cap= "A. Histogramme et B. graphique de densité montrant la distribution de la taille de zooplancton étudié par analyse d'image.", echo=FALSE, message=FALSE}
a <- chart(data = zooplankton, ~ size) +
...
```{r, fig.cap="Distribution des tailles au sein de l'échantillon de zooplancton."}
# Importation du jeu de données
(zooplankton <- read("zooplankton", package = "data.io", lang = "fr"))
# Réalisation du graphique
chart(data = zooplankton, ~ size) +
...
```{r, fig.cap="Distribution des tailles pour 4 groupes taxonomiques de zooplancton."}
# Importation du jeu de données
zooplankton <- read("zooplankton", package = "data.io", lang = "fr")
...
```{r, fig.cap="Distribution de tailles des 17 classes d'organismes planctoniques (diagramme en violon)."}
...
```{r, fig.cap="Distribution de tailles des 17 classes d'organismes planctoniques (diagramme en violon avec l'ajout de la fonction `coord_flip()`)."}
chart(data = zooplankton, size ~ class) +
  geom_violin() +
  coord_flip()
```
...
```{r, fig.cap="Distribution des tailles des 17 classes d'organismes planctoniques (sous forme de graphique en lignes de crêtes).", message=FALSE}
