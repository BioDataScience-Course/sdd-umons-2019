...
```{r, fig.cap= "Dénombrement du individus homme et femme dans l'étude sur l'obésité en Hainaut en tenant compte des années de mesures.", echo = TRUE}
...
```{r, fig.cap="Instructions pour obtenir un graphique en barres indiquant les moyennes par groupe."}
# Réalisation du graphique
chart(copepoda, formula = size ~ class) +
  stat_summary(geom = "col", fun.y = "mean")
```
...
```{r, fig.cap="Comparaison entre le graphique en barres et le nuage de points montrant tous les deux la taille moyenne en fonction des classes de copépodes."}
...
```{r, fig.cap= "Instructions pour obtenir un graphique en cammenbet."}
chart(copepoda, formula = ~ factor(0) %fill=% class) +
  geom_bar(width = 1) + 
  coord_polar("y", start = 0)+
  labs( x = "", y = "") + 
...
```{r, fig.cap= "Dénombrement des niveaux d'une variable facteur."}
