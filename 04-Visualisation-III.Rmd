...
SciViews::R
# Not used?!
e_mathaei <- data_frame(id = 1:345, color = rep(c("black", "purple", "brown", "green"),
  times = c(236, 40, 35, 34)))
```
...
```{r , fig.cap= "Exemple d'un graphique en barres montrant le dénombrement des niveaux d'une variable facteur, avec les éléments importants du graphique mis en évidence en couleurs.", echo=FALSE, message=FALSE}
...
```{r, fig.cap="Abondances de quatres types de copépodes dans un échantillon de zooplancton."}
...
```{r, fig.cap="Dénombrement des hommes (M) et des femmes (W) dans l'étude sur l'obésité en Hainaut en tenant compte des années de mesure pour (B).", echo=TRUE}
...
```{r, fig.cap="Dénombrement des hommes (M) et des femmes (W) dans l'étude sur l'obésité en Hainaut en tenant compte des années de mesure (différentes présentations).", echo=TRUE}
...
```{r, fig.cap="Dénombrement des classes du jeu de données zooplankton (version avec barres horizontales)."}
chart(data = zooplankton, ~ class) +
  geom_bar() +
  ylab("Dénombrement") +
  coord_flip()
...
```{r, fig.cap="Exemple de graphique en barres représentant les moyennes de tailles par groupe zooplanctonique."}
...
error <- data_frame(
  index = 1:347,
  fact  = as.factor(rep(letters[1:10],
...
```{r, fig.cap="Nuage de points univarié.", echo=FALSE}
df <- data_frame(x = x)
chart(data = df, x ~ 0) +
  geom_point() +
...
```{r, fig.cap="Distribution des tailles par groupes taxonomiques pour le zooplancton."}
chart(data = copepoda, size ~ class) +
  geom_boxplot()
```
...
```{r , fig.cap= "Piège des boites de dispersion : trop peu d'observations disponibles pour `a`.", echo= FALSE}
error <- data_frame(fact  = rep(c("a", "b", "c"), times = c(4, 11, 8)), 
                    value = c(55, 83, 120, 26,
                              1, 71, 55, 68, 78, 60, 83, 120, 82, 53, 26,
                              1, 71, 78, 60, 83, 120, 53, 26))
chart(data = error, value ~ fact) +
  geom_boxplot() +
...
```{r, fig.cap="Taille de copépodes pour différents groupes taxonomiques (le nombre d'observations est indiqué au dessus de chaque boite)."}
give_n <- function(x)
...
```{r, fig.cap= "Croissance de poulets en utilisant quatre aliments différents (1-4)."}
...
```{r , fig.cap= "A) Masse d'oursins en fonction de leur taille  et de leur origine. B) Masse totale en fonction de la masse des parties solides de ces mêmes oursins."}
...
```{r, fig.cap="Graphique exemple réalisé avec le moteur graphique R de base."}
...
```{r, fig.keep=2, fig.cap="Graphique exemple réalisé avec le moteur graphique de base et la fonction `chart()`."}
chart$base({
  par(mar = c(5.1, 4.1, 4.1, 6.1))
  plot(urchin$weight, urchin$height,
    col = scales::hue_pal()(2)[urchin$origin], pch = 19, cex = 0.8,
...
```{r, fig.cap="Graphique à facettes avec le moteur graphique de base."}
